...
def jacobi_diis(
    ...
    diis_max_subspace : int
        Maximum number of error vectors to keep in DIIS.
    diis_start_iter: int
        Number of iterations at which to start DIIS mixing.
    """
    if callback is None:
    ...
    diis = DIIS(max_subspace=diis_max_subspace, start_iter=diis_start_iter)
    while state.n_iter < max_iter:
        ...
        if state.n_iter == max_iter:
            raise la.LinAlgError(
                "Maximum number of iterations (== " + str(max_iter) + " reached in Jacobi/DIIS "
                "procedure."
            )

        jacobi_step = evaluate(Dinv @ state.residual)
        if explicit_symmetrisation:
            jacobi_step = explicit_symmetrisation.symmetrise(jacobi_step)
        x_new = evaluate(state.solution + jacobi_step)
        state.solution = diis.extrapolate(x_new, jacobi_step)
