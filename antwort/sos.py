    ...
    for i, dip in enumerate(state.transition_dipole_moment):
        for c1 in range(3):
            for c2 in range(c1, 3):
                sos[c1, c2] += 2.0 * (dip[c1] * dip[c2]) /\
                               state.excitation_energy[i]
                sos[c2, c1] = sos[c1, c2]
    return sos
...
def sos_complex_polarizability(state, omegas=None, gamma=0.01):
    ...
    for i, dip in enumerate(state.transition_dipole_moment):
        for c1 in range(3):
            for c2 in range(c1, 3):
                sos[:, c1, c2] += (
                    dip[c1] * dip[c2] / (state.excitation_energy[i] -
                                         omegas + np.complex(0, -gamma))
                    + dip[c2] * dip[c1] / (state.excitation_energy[i] +
                                           omegas + np.complex(0, gamma))
                )
                sos[:, c2, c1] = sos[:, c1, c2]
    return sos
